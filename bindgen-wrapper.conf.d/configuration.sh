...
preprocess_before_headersFolderPath()
{
	...
	cd "$dpdkSrcDir" 1>/dev/null 2>/dev/null
		...
		CFLAGS="-I$configurationFolderPath/musl-fixes" PATH="$pathWithGnuCoreUtils" make install T=x86_64-native-bsdapp-clang DESTDIR="$dpdkDestDir" prefix=/usr/local O="$dpdkBuildDir"
